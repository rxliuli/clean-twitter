...
function mergeAndUnique(arr1: any, arr2: any) {
  const arr = [...arr1, ...arr2]

  const uniqueArr = [...new Set(arr.map((item: any) => item.screen_name))]

  const result = uniqueArr.map((name) => {
    const item1 = arr1.find((item: any) => item.screen_name === name)
    const item2 = arr2.find((item: any) => item.screen_name === name)
    return { ...item1, ...item2 }
  })
    ...
    conversationEntries
      .flatMap((entry: any) =>
        entry.content.items.map(
          (it: any) => it.item.itemContent.tweet_results.result,
        ),
      )
